...
class SpeedkinoAiPage extends StatefulWidget {
  ...
  @override
  State<SpeedkinoAiPage> createState() => _SpeedkinoAiPageState();
}

class _SpeedkinoAiPageState extends State<SpeedkinoAiPage>
    with SingleTickerProviderStateMixin {
  final _dataService = SpeedkinoDataService();
  late TabController _tabController;

  bool _loading = true;
  String? _error;
  SpeedkinoAnalysisResult? _analysis;
  ...
  Future<void> _loadData() async {
    try {
      ...
      final analyzer = SpeedkinoAnalyzer(_dataService.draws);
      setState(() {
  ...
  Widget _buildRecCard(SpeedkinoRecommendation rec) {
    return Container(
        ...
        children: [
          ...
          Wrap(
            ...
            children: rec.numbers.map((n) {
              ...
              return _KinoBall(number: n, color: c);
            }).toList(),
          ),
        ],
      ),
  ...
  Widget _buildBallWrap(List<int> numbers) {
    return Wrap(
      ...
      runSpacing: 6,
      children: numbers
          .map((n) => _KinoBall(number: n, color: _ballColor(n), size: 36))
          .toList(),
    );
  }
    ...
    return Column(
      children: top.map((e) {
        ...
        return Padding(
          ...
          child: Row(
            children: [
              _KinoBall(number: e.key, color: c, size: 28),
          ...
          Expanded(
            child: Stack(
              children: [
                ...
                FractionallySizedBox(
                  widthFactor: (percent / 20).clamp(0, 1),
                  child: Container(
                    height: 20,
                    decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(4),
...
class _KinoBall extends StatelessWidget {
  final int number;
  final Color color;
  final double size;
