...
class TreasureDataService {
  ...
  static const _cacheKey = 'treasure_draws_cache';
  static const _lastRoundKey = 'treasure_last_round';

  final List<TreasureDraw> _draws = [];
  bool _loaded = false;

  List<TreasureDraw> get draws => List.unmodifiable(_draws);
  ...
  Future<void> loadData({int fetchCount = 300}) async {
    ...
    if (cached != null) {
      try {
        ...
        _draws.addAll(
          list.map((e) => TreasureDraw.fromJson(e as Map<String, dynamic>)),
        );
      } catch (_) {}
    }
  ...
  String _buildUrl(int round) {
    final base = '$_api?method=getTreasureHunterNumber&drwNo=$round';
  ...
  Future<TreasureDraw?> _fetchDraw(int round) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (json['returnValue'] == 'success') {
          return TreasureDraw.fromJson(json);
        }
      }
    } catch (_) {}
    return null;
  ...
  static List<TreasureDraw> get _fallbackDraws {
    final rng = Random(351035);
    return List.generate(300, (i) {
      final round = 300 - i;
      final pool = List.generate(35, (j) => j + 1)..shuffle(rng);
      return TreasureDraw(
        round: round,
        numbers: pool.sublist(0, 6)..sort(),
        treasureNumber: rng.nextInt(10) + 1,
      );
    });
  }
}
