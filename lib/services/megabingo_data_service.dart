...
class MegabingoDataService {
  static const _api = 'https://www.dhlottery.co.kr/common.do';
  static const _corsProxy = 'https://corsproxy.io/?';
  static const _cacheKey = 'megabingo_draws_cache';
  static const _lastRoundKey = 'megabingo_last_round';

  final List<MegabingoDraw> _draws = [];
  bool _loaded = false;

  List<MegabingoDraw> get draws => List.unmodifiable(_draws);
  ...
  Future<void> loadData({int fetchCount = 300}) async {
    ...
    if (cached != null) {
      try {
        ...
        _draws.addAll(
          list.map((e) => MegabingoDraw.fromJson(e as Map<String, dynamic>)),
        );
      } catch (_) {}
    }
  ...
  String _buildUrl(int round) {
    final base = '$_api?method=getMegaBingoNumber&drwNo=$round';
  ...
  Future<MegabingoDraw?> _fetchDraw(int round) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (json['returnValue'] == 'success') {
          return MegabingoDraw.fromJson(json);
        }
      }
    } catch (_) {}
    return null;
  ...
  static List<MegabingoDraw> get _fallbackDraws {
    final rng = Random(402040);
    return List.generate(300, (i) {
      final round = 300 - i;
      final pool = List.generate(40, (j) => j + 1)..shuffle(rng);
      return MegabingoDraw(round: round, numbers: pool.sublist(0, 20)..sort());
    });
  }
}
