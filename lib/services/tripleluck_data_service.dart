...
class TripleluckDataService {
  static const _api = 'https://www.dhlottery.co.kr/common.do';
  static const _corsProxy = 'https://corsproxy.io/?';
  static const _cacheKey = 'tripleluck_draws_cache';
  static const _lastRoundKey = 'tripleluck_last_round';

  final List<TripleluckDraw> _draws = [];
  bool _loaded = false;

  List<TripleluckDraw> get draws => List.unmodifiable(_draws);
  ...
  Future<void> loadData({int fetchCount = 300}) async {
    ...
    if (cached != null) {
      try {
        ...
        _draws.addAll(
          list.map((e) => TripleluckDraw.fromJson(e as Map<String, dynamic>)),
        );
      } catch (_) {}
    }
  ...
  String _buildUrl(int round) {
    final base = '$_api?method=getTripleLuckNumber&drwNo=$round';
  ...
  Future<TripleluckDraw?> _fetchDraw(int round) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (json['returnValue'] == 'success') {
          return TripleluckDraw.fromJson(json);
        }
      }
    } catch (_) {}
    return null;
  ...
  static List<TripleluckDraw> get _fallbackDraws {
    final rng = Random(270327);
    return List.generate(300, (i) {
      final round = 300 - i;
      final pool = List.generate(27, (j) => j + 1)..shuffle(rng);
      return TripleluckDraw(
        round: round,
        tripleNumbers: (pool.sublist(0, 3)..sort()),
        luckNumbers: (pool.sublist(3, 6)..sort()),
      );
    });
  }
}
