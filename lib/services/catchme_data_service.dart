...
class CatchmeDataService {
  static const _api = 'https://www.dhlottery.co.kr/common.do';
  static const _corsProxy = 'https://corsproxy.io/?';
  static const _cacheKey = 'catchme_draws_cache';
  static const _lastRoundKey = 'catchme_last_round';

  final List<CatchmeDraw> _draws = [];
  bool _loaded = false;

  List<CatchmeDraw> get draws => List.unmodifiable(_draws);
  ...
  Future<void> loadData({int fetchCount = 300}) async {
    ...
    if (cached != null) {
      try {
        ...
        _draws.addAll(
          list.map((e) => CatchmeDraw.fromJson(e as Map<String, dynamic>)),
        );
      } catch (_) {}
    }
  ...
  String _buildUrl(int round) {
    final base = '$_api?method=getCatchMeNumber&drwNo=$round';
  ...
  Future<CatchmeDraw?> _fetchDraw(int round) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (json['returnValue'] == 'success') {
          return CatchmeDraw.fromJson(json);
        }
      }
    } catch (_) {}
    return null;
  ...
  static List<CatchmeDraw> get _fallbackDraws {
    final rng = Random(451145);
    return List.generate(300, (i) {
      final round = 300 - i;
      return CatchmeDraw(round: round, drawnNumber: rng.nextInt(45) + 1);
    });
  }
}
